...
// dbo.tblemployee set, the same week-off exclusion and the same isLateRow test.
...
app.get('/api/hr/late-employees', authenticate, requireRole('HR'), requireDbConfig, async (req, res) => {
  ...
  try {
    const pool = await getPool();
    const [empResult, regResult] = await Promise.all([
      pool.request().query(`SELECT LTRIM(RTRIM(e.paycode)) AS paycode, LTRIM(RTRIM(e.empname)) AS empname,
    ...
    const employees = [];
    for (const emp of empResult.recordset) {
      const lateRows = usableRegisterRows(rowsByPay.get(String(emp.paycode).trim()) || []).filter(isLateRow);
      if (!lateRows.length) continue;
      // Most recent Late register row in the range is the representative record;
      // lateDays keeps the full number of Late rows for that employee.
      const record = lateRows.slice().sort((a, b) => new Date(b.dateoffice) - new Date(a.dateoffice))[0];
      ...
      employees.push({
        ...
        date: record.dateoffice, dateoffice: record.dateoffice, shift: record.shift,
        in1: record.in1, in2: record.in2, out1: record.out1, out2: record.out2,
        inTime: normalized.inTime, outTime: normalized.outTime,
        hoursworked: record.hoursworked, otduration: record.otduration ?? null,
        // dbo.tbltimeregister.latearrival is the register's own Late duration in
        // minutes; it is passed through untouched and never recalculated here.
        latearrival: Number(record.latearrival || 0), lateDays: lateRows.length,
        status: record.status, statusCode: normalized.statusCode, statusLabel: normalized.statusLabel,
        isLate: true, reason: record.reason
      });
    }
